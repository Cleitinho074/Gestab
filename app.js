// ══════════════════════════════════════
// HELPERS DE VARIAÇÃO DE PRODUTO
// (antes viviam soltos num <script> dentro do <head> do index.html)
// ══════════════════════════════════════
...
function getBusinessLimit(type){ const b = BUSINESS_TYPES[type]; return b ? b.limit : MEI_LIMIT; } // null = sem limite (proposital, não confundir com "tipo desconhecido")
function currentLimit(){ return getBusinessLimit(state.businessType||'MEI'); }
...
async function loadDashboardStats(){
  ...
  if(data){
    ...
    { const bl = getBusinessLimit(state.businessType); state.meiLimit = bl !== null ? bl : (data.mei_limit != null ? +data.mei_limit : null); }
...
async function demoLogin(){
  const btn = document.querySelector('a[onclick="demoLogin()"]');
  if(btn){ btn.textContent = '⏳ Iniciando...'; btn.style.pointerEvents='none'; }

  // Recupera ou gera o device_token exclusivo deste aparelho
  let deviceToken = localStorage.getItem('mei_device_token');

  try {
    const data = await apiCall(
      '/api/auth/demo-session', 'POST',
      { device_token: deviceToken || null },
      false // não requer auth
    );
    if(!data){ throw new Error('Sem resposta'); }

    // Salva o device_token para futuras visitas
    if(data.device_token){
      localStorage.setItem('mei_device_token', data.device_token);
    }
    // Salva o JWT e entra
    localStorage.setItem('mei_jwt', data.token);
    await loginSuccess(data.user);
  } catch(e) {
    toast('Erro ao iniciar conta demo. Tente novamente.', 'error');
    if(btn){ btn.textContent = '🎯 Entrar com conta demo'; btn.style.pointerEvents=''; }
  }
}
...
function renderDashboard(el){
  ...
  const pdfLimit = (() => {
    const l = getBusinessLimit(state.businessType||'MEI');
    return (l === null || l === undefined) ? null : l;
  })();
  const hasLimit = pdfLimit !== null;
  const pct = hasLimit ? Math.min(100,(state.yearRevenue/pdfLimit)*100) : 0;
  const remaining = hasLimit ? pdfLimit-state.yearRevenue : null;
  const alertClass=pct>=90?'danger':pct>=75?'warning':'';
  const alertMsg = hasLimit ? (pct>=90
    ?`⚠️ ATENÇÃO: Você está a apenas ${R(remaining)} do limite. Consulte um contador urgentemente.`
    :pct>=75?`🔔 Atenção: Você atingiu ${pct.toFixed(0)}% do limite anual (${R(pdfLimit)}). Fique atento.`:'') : '';
  ...
  el.innerHTML=`
    <div class="limit-card" title="${getBusinessLabel(state.businessType||'MEI')}">
      <div class="limit-header">
        ...
        ${hasLimit?`<span class="limit-pct ${pct>=90?'red':pct>=75?'amber':'green'}">${pct.toFixed(1)}%</span>`:`<span class="limit-pct muted">Sem limite</span>`}
      </div>
      ${hasLimit?`<div class="progress-track"><div class="progress-fill ${alertClass}" style="width:${pct}%"></div></div>`:''}
      <div class="limit-info">
        <span>Faturado: <strong>${R(state.yearRevenue)}</strong></span>
        <span>Limite: <strong>${hasLimit?R(pdfLimit):'Não definido'}</strong></span>
        ${hasLimit?`<span>Restante: <strong>${R(remaining)}</strong></span>`:''}
  ...
  el.innerHTML=`
    <div class="section-header">
      ...
      <div style="display:flex;gap:8px">
        <button class="btn btn-ghost btn-sm" onclick="openSalaryCalc()" title="Calculadora de salário com impostos">🧮 Calcular Salário</button>
...
function filterProductsList(){
  ...
  return search
    ? state.products.filter(p => {
        ...
        if((p.type==='service'?'serviço':'produto').includes(search)) return true;
        if(p.variations && p.variations.some(v=>v.name.toLowerCase().includes(search))) return true;
        return false;
      })
    : state.products;
}
...
function renderProducts(el){
  ...
  el.innerHTML=`
    <div class="section-header">
      <span class="section-title" id="products-count">📦 Produtos & Serviços (${filtered.length}/${state.products.length})</span>
      <div style="display:flex;gap:8px;align-items:center">
        <input id="product-search-input" value="${state.productSearch||''}"
          placeholder="🔍 Buscar produto..." style="padding:6px 12px;border:1.5px solid var(--border);border-radius:8px;background:var(--bg);color:var(--text);font-size:13px;width:180px"
...
// Atualiza SOMENTE a contagem e o corpo da tabela — nunca recria o campo de busca,
// senão o input perde o foco a cada letra digitada (o que travava a digitação e
// fazia a barra de espaço "rolar" a página, já que o foco ia parar no body).
function filterProducts(){
  ...
  if(countEl) countEl.textContent = `📦 Produtos & Serviços (${filtered.length}/${state.products.length})`;
  if(tbody) tbody.innerHTML = filtered.map(productRowHtml).join('');
}
...
function reportTab(tab,el){
  ...
  if(tab==='funcionarios')c.innerHTML=reportFuncionarios();
...
function openHoleriteModal(employeeId){
  ...
  openModal(`
    <div class="modal-title">🧾 Holerite — ${e.name} (${mesAtualNome})</div>
    ${holeriteTableHtml(e)}
    <div class="form-actions">
      <button class="btn btn-ghost" onclick="_closeModal()">Fechar</button>
      <button class="btn btn-primary" onclick="printHolerite('${e.id}')">🖨️ Imprimir / Salvar PDF</button>
    </div>`,true);
}
function printHolerite(employeeId){
  const e = state.employees.find(x=>x.id===employeeId);
  if(!e) return;
  const now = new Date();
  ...
  const html = `
    <div class="abnt-cover">
      ...
      <br/><br/><br/><br/>
      <p><strong style="font-size:14pt">HOLERITE / CONTRACHEQUE</strong></p>
      <p><strong>${e.name}${e.role?' — '+e.role:''}</strong></p>
      <p><strong>Competência: ${mesAtualNome}</strong></p>
      <br/><br/><br/><br/><br/><br/><br/><br/><br/>
      <p>Campo Grande – MS</p>
      <p>${dataEmissao}</p>
    </div>
    <div class="abnt-section">
      <h2>1 HOLERITE / CONTRACHEQUE — ${mesAtualNome.toUpperCase()}</h2>
      ${holeriteTableHtml(e)}
    </div>
    <div class="abnt-section">
      <hr/>
      <p style="font-size:10pt;text-align:center">Documento gerado pelo sistema MEI Fácil em ${dataEmissao} às ${now.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}.</p>
    </div>`;
...
function reportMEI(){
  ...
  const pct = hasLimit ? (state.yearRevenue/LIMIT*100) : 0;
  const meses=new Date().getMonth()+1;
  const projectedAnnual=(state.yearRevenue/meses)*12;
  return `
    <div class="card" style="margin-bottom:16px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
        <span style="font-weight:700;font-size:15px">🏛️ Situação ${getBusinessLabel(state.businessType||'MEI')} ${new Date().getFullYear()}</span>
        <button class="btn btn-ghost btn-sm" onclick="openEditRevenueModal()">✏️ Corrigir faturamento</button>
      </div>
      ${hasLimit?`<div class="progress-track" style="height:20px"><div class="progress-fill ${pct>=90?'danger':pct>=75?'warning':''}" style="width:${Math.min(100,pct)}%"></div></div>`:''}
      <div style="margin-top:8px;font-size:13px;display:flex;justify-content:space-between;flex-wrap:wrap;gap:4px">
        <span>Faturado: <strong>${R(state.yearRevenue)}</strong></span>
        ${hasLimit?`<span>${pct.toFixed(1)}% utilizado</span><span>Limite: <strong>${R(LIMIT)}</strong></span>`:`<span>Sem limite de faturamento definido para este tipo de empresa</span>`}
      </div>
    </div>
    <div class="cards-grid">
      ...
      ${hasLimit?`<div class="card card-metric"><div class="card-label">Projeção anual</div><div class="card-value ${projectedAnnual>LIMIT?'red':'green'}">${R(projectedAnnual)}</div><div class="card-sub">${projectedAnnual>LIMIT?'⚠️ Pode ultrapassar!':'✅ Dentro do limite'}</div></div>`:`<div class="card card-metric"><div class="card-label">Projeção anual</div><div class="card-value blue">${R(projectedAnnual)}</div><div class="card-sub">Sem teto aplicável</div></div>`}
...
function reportPDF(){
  ...
  return `
    <div class="card" style="max-width:700px;margin:0 auto">
      ...
      <div style="margin-bottom:20px">
        ...
        ${tipos.map(t=>`
          <label style="display:flex;align-items:flex-start;gap:12px;padding:12px 16px;border:1.5px solid var(--border);border-radius:8px;margin-bottom:8px;cursor:pointer;transition:.15s"
            ...
            <input type="radio" name="report-type" value="${t.id}" ${t.id==='mensal'?'checked':''} onchange="toggleReportSelectors()" style="margin-top:3px;flex-shrink:0"/>
            <div>
      ...
      <div class="form-group" id="report-employee-box" style="display:none;background:var(--blue-x);padding:14px 16px;border-radius:8px">
        <label>🧑‍💼 Funcionário específico (opcional)</label>
        <select id="report-employee-filter">
          <option value="">Todos os funcionários (relatório geral da folha)</option>
          ${state.employees.map(e=>`<option value="${e.id}">${e.name}${e.role?' — '+e.role:''}</option>`).join('')}
        </select>
        <div style="font-size:12px;color:var(--muted);margin-top:6px">Selecionando um funcionário, o relatório traz os dados cadastrais, o desempenho de vendas dele (dentro do período escolhido abaixo) e o holerite calculado com base no salário registrado.</div>
      </div>
...
function toggleReportSelectors(){
  ...
  const clientBox = document.getElementById('report-client-box');
  if(clientBox) clientBox.style.display = tipo==='clientes' ? 'block' : 'none';
  const employeeBox = document.getElementById('report-employee-box');
  if(employeeBox) employeeBox.style.display = tipo==='funcionarios' ? 'block' : 'none';
}
// Mantém o nome antigo funcionando, caso algo ainda o referencie
function toggleClientReportSelector(){ toggleReportSelectors(); }
...
function gerarPDF(){
  ...
  const funcionarioFiltroId = tipo==='funcionarios' ? (document.getElementById('report-employee-filter')?.value || '') : '';
  const funcionarioFiltro = funcionarioFiltroId ? state.employees.find(e=>e.id===funcionarioFiltroId) : null;
  ...
  const pctMEI = pdfLimit !== null ? (state.yearRevenue/pdfLimit*100).toFixed(1) : '0.0';
  const meses  = new Date().getMonth()+1;
  ...
  const nomes = {
    ...
    funcionarios: funcionarioFiltro ? `RELATÓRIO DE FUNCIONÁRIO — ${funcionarioFiltro.name.toUpperCase()}` : 'RELATÓRIO DE FUNCIONÁRIOS E FOLHA DE PAGAMENTO',
  ...
  if(tipo==='funcionarios'){
    const holeriteHtml = holeriteTableHtml;
    if(funcionarioFiltro){
      const vendasFunc = vendasPeriodo.filter(s=>s.status==='paid' && s.employeeId===funcionarioFiltro.id);
      const totalVendidoFunc = vendasFunc.reduce((a,s)=>a+s.netTotal,0);
      const ticketMedioFunc = vendasFunc.length ? totalVendidoFunc/vendasFunc.length : 0;
      seccoes += `
        <h2 class="abnt-first-section">1 DADOS CADASTRAIS</h2>
        <table>
          <tr><th>Campo</th><th>Valor</th></tr>
          <tr><td>Nome</td><td>${funcionarioFiltro.name}</td></tr>
          <tr><td>Função</td><td>${funcionarioFiltro.role||'—'}</td></tr>
          <tr><td>Telefone</td><td>${funcionarioFiltro.phone||'—'}</td></tr>
          <tr><td>Dia de pagamento</td><td>Todo dia ${funcionarioFiltro.paymentDay}</td></tr>
          <tr><td>Salário Bruto registrado</td><td>${R(funcionarioFiltro.salary)}</td></tr>
        </table>
        <h2 class="abnt-section">2 DESEMPENHO DE VENDAS (${fmtPeriodo})</h2>
        <table>
          <tr><th>Indicador</th><th>Valor</th></tr>
          <tr><td>Total vendido</td><td>${R(totalVendidoFunc)}</td></tr>
          <tr><td>Quantidade de vendas pagas</td><td>${vendasFunc.length}</td></tr>
          <tr><td>Ticket médio</td><td>${R(ticketMedioFunc)}</td></tr>
        </table>
        <h2 class="abnt-section">3 HOLERITE / CONTRACHEQUE (${mesAtualNome.toUpperCase()})</h2>
        ${holeriteHtml(funcionarioFiltro)}`;
    } else {
      seccoes += `
        <h2 class="abnt-first-section">1 CADASTRO DE FUNCIONÁRIOS</h2>
        <table>
          <tr><th>Nome</th><th>Função</th><th>Telefone</th><th>Dia Pagto.</th><th>Salário Bruto (R$)</th></tr>
          ${state.employees.map(e=>`<tr>
            <td>${e.name}</td><td>${e.role||'—'}</td><td>${e.phone||'—'}</td>
            <td>Dia ${e.paymentDay}</td><td>${R(e.salary)}</td>
          </tr>`).join('') || '<tr><td colspan="5">Nenhum funcionário cadastrado</td></tr>'}
        </table>
        <h2 class="abnt-section">2 HOLERITE / FOLHA DE PAGAMENTO CONSOLIDADA (${mesAtualNome.toUpperCase()})</h2>
        ${state.employees.map(e=>`<h3>${e.name}${e.role?' — '+e.role:''}</h3>${holeriteHtml(e)}`).join('') || '<p>Nenhum funcionário cadastrado</p>'}`;
    }
  }
  ...
  if(tipo==='mei'){
    ...
    seccoes += `
      <h2 class="abnt-first-section">1 SITUAÇÃO DO ENQUADRAMENTO — ${getBusinessLabel(state.businessType||'MEI').toUpperCase()}</h2>
      <table>
        <tr><th>Indicador</th><th>Valor</th></tr>
        <tr><td>Tipo de empresa</td><td>${getBusinessLabel(state.businessType||'MEI')}</td></tr>
        <tr><td>Faturamento Acumulado no Ano</td><td>${R(state.yearRevenue)}</td></tr>
        ${pdfLimit!==null
          ? `<tr><td>Limite Anual (${getBusinessLabel(state.businessType||'MEI')})</td><td>${R(pdfLimit)}</td></tr>
             <tr><td>Percentual Utilizado</td><td>${pctMEI}%</td></tr>
             <tr><td>Saldo Disponível</td><td>${R(pdfLimit-state.yearRevenue)}</td></tr>
             <tr><td>Projeção Anual (base ${meses} meses)</td><td>${R(proj)}</td></tr>
             <tr><td>Situação</td><td>${proj>pdfLimit?'⚠️ RISCO DE DESENQUADRAMENTO':'✅ DENTRO DO LIMITE'}</td></tr>`
          : `<tr><td>Limite de Faturamento</td><td>Não se aplica (tipo sem teto)</td></tr>
             <tr><td>Projeção Anual (base ${meses} meses)</td><td>${R(proj)}</td></tr>`
        }
      </table>
  ...
  const html = `
    ...
    <div class="abnt-section">
      ...
      <table>
        ...
        <tr><td>Faturamento acumulado no ano (${getBusinessLabel(state.businessType||'MEI')})</td><td>${R(state.yearRevenue)}</td></tr>
        ${pdfLimit!==null?`<tr><td>Limite anual (${getBusinessLabel(state.businessType||'MEI')})</td><td>${R(pdfLimit)}</td></tr>`:'<tr><td>Tipo sem limite de faturamento definido</td><td>—</td></tr>'}
        ${pdfLimit!==null?`<tr><td>Percentual do limite utilizado</td><td>${pctMEI}%</td></tr>`:''}
      </table>
...
const IRRF_TABLE_2026 = [
  ...
  { teto: 3751.05,  aliq: 0.15,  deducao: 394.16  },
  { teto: 4664.68,  aliq: 0.225, deducao: 675.49  },
...
function calcINSS(salarioBruto){
  ...
  for(const faixa of INSS_TABLE_2026){
    if(base <= 0) break;
    const teto = faixa.teto;
    const tribut = Math.min(base, teto - prev);
    inss += tribut * faixa.aliq;
    prev = teto;
    base -= tribut;
...
// Calcula o holerite de um funcionário com base no salário REAL cadastrado
function computeHolerite(e){
...
function holeriteTableHtml(e){
  ...
  return `
    <table>
      ...
      <tr><td>Salário Bruto</td><td>${R(h.bruto)}</td></tr>
      <tr><td>(-) INSS</td><td>-${R(h.inss)}</td></tr>
      ${h.irrf>0?`<tr><td>(-) IRRF</td><td>-${R(h.irrf)}</td></tr>`:''}
      <tr><td><strong>Salário Líquido a Receber</strong></td><td><strong>${R(h.liquido)}</strong></td></tr>
      <tr><td colspan="2"><strong>Encargos do Empregador (não descontados do funcionário)</strong></td></tr>
      ${h.encargos.inssPatronal>0?`<tr><td>INSS Patronal</td><td>${R(h.inssPatr)}</td></tr>`:''}
      ${h.encargos.rat>0?`<tr><td>RAT</td><td>${R(h.rat)}</td></tr>`:''}
      ${h.encargos.terceiros>0?`<tr><td>Sistema S (Terceiros)</td><td>${R(h.terc)}</td></tr>`:''}
      <tr><td>FGTS (8%)</td><td>${R(h.fgts)}</td></tr>
      <tr><td><strong>Custo Total para a Empresa</strong></td><td><strong>${R(h.custoTotal)}</strong></td></tr>
    </table>
    <div style="font-size:11px;margin-top:4pt">ℹ️ ${h.encargos.obs} Cálculo de IRRF sem dependentes declarados; ajuste com um contador se houver.</div>`;
}
  ...
  openModal(`
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px">
      <div class="form-group">
        ...
        <input type="number" id="sc-bruto" value="${SALARIO_MINIMO_2026.toFixed(2)}" min="0" step="0.01" oninput="updateSalaryCalc()"/>
      </div>
      <div class="form-group">
        <label>Dependentes (IRRF)</label>
        <input type="number" id="sc-dep" value="0" min="0" max="20" oninput="updateSalaryCalc()"/>
      </div>
    </div>
    <div class="form-group">
...
function updateSalaryCalc(){
  const bruto    = parseFloat(document.getElementById('sc-bruto')?.value) || 0;
  const dep      = parseInt(document.getElementById('sc-dep')?.value)     || 0;
  const vtBruto  = parseFloat(document.getElementById('sc-vt')?.value)    || 0;
  ...
  el.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
      <!-- COLUNA FUNCIONÁRIO -->
      <div>
        <div style="font-weight:700;font-size:13px;color:var(--text);margin-bottom:8px;padding-bottom:6px;border-bottom:1px solid var(--border)">
        ...
        <div style="font-size:13px">
          ...
          <div style="display:flex;justify-content:space-between;padding:5px 0;color:var(--red-l)">
            <span>(-) INSS ${((inss/bruto)*100||0).toFixed(1)}%</span><span>-${R(inss)}</span>
          </div>
          ...
          <div style="display:flex;justify-content:space-between;padding:8px 0;margin-top:4px;background:var(--green-x);border-radius:6px;padding:6px 10px;font-weight:700;color:var(--green-l)">
            <span>💰 Salário Líquido</span><span>${R(liquido)}</span>
          </div>
        </div>
      </div>
      <!-- COLUNA EMPREGADOR -->
      <div>
        ...
        <div style="font-size:13px">
          ...
          ${terc>0?`<div style="display:flex;justify-content:space-between;padding:5px 0;color:var(--amber)">
            <span>(+) Sistema S ${(encargos.terceiros*100).toFixed(1)}%</span><span>+${R(terc)}</span>
          </div>`:''}
...
function filterProductsLive(val){
  state.productSearch = val;
  const search = val.toLowerCase().trim();
  const filtered = search
    ? state.products.filter(p => {
        if(p.name.toLowerCase().includes(search)) return true;
        if(p.type === 'product' ? 'produto'.includes(search) : 'serviço'.includes(search)) return true;
        if(p.variations && p.variations.some(v => v.name.toLowerCase().includes(search))) return true;
        return false;
      })
    : state.products;
...
function renderAdminSeed(el){
  const isDemo = state.user?.is_demo === true;
  if(!isDemo){
